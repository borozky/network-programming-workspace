...
public class GameManager {
	...
	/**
	 * Sets the number of digits for the whole game
	 * @param numDigits
	 */
	...
	/**
	 * Check if player has forfeited the current round
	 * @return
	 */
	 ...
	 * 
	 * A new secret code is created on each new round created. 
	 * When every time a secret code is created, it will fire a onSecretCodeCreated() event.
	...
	public void addGuess(String guess) {
		...
		if (guess.trim().equals("f")) {
			...
			// Round might be ended by player forfeiting, so do a check
		...
		// the 10th guess was incorrect (player loses)
		if (playerLoses()){
